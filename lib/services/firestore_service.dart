...
class FirestoreService {
  ...
  Stream<firestore.DocumentSnapshot<Map>> recommendationsStream(
      String? userID) {
    return _firebaseFirestore
        .collection("users")
        .doc(userID)
  ...
  Stream<List<String>> fetchMatches(String userID) {
    final streamOne = _firebaseFirestore
        ...
        .map((QuerySnapshot snapshot) =>
            snapshot.docs.map((e) => e['uid2'] as String).toList());
    final streamTwo = _firebaseFirestore
        .collection("matches")
        .where("uid2", isEqualTo: userID)
        .snapshots()
        .map((QuerySnapshot snapshot) =>
            snapshot.docs.map((e) => e['uid1'] as String).toList());
